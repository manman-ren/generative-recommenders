...
def _ragged_hstu_attn_fwd(  # noqa C901
    ...
    BLOCK_M: tl.constexpr,
    BLOCK_N: tl.constexpr,
    NUM_LAUNCH: tl.constexpr,
):
    # each invocation goes through the seq_len (i.e with pid(0) being 0 to num_programs(0)
    mGrid_len = (2128 + BLOCK_M - 1) // BLOCK_M #triton.cdiv(MAX_SEQ_LEN, BLOCK_M)
    start_id = tl.program_id(0)
    # NUM_LAUNCH means total number of launch grids, will use 1D launch grid
    # assign work to each launch grid in round-robin, map to 3D data grid of [H, B, mGrid]
    num_data_grid = mGrid_len * H * Z
    # each instance gets 2 data grids [off_h, off_z, mGrid] and [off_h+1, off_z, mGrid_len-1-mGrid]
    x= tl.num_programs(0)
    for data_id in range(start_id, num_data_grid, x):
        # let x be num_programs(0), y be start_id
        # when (data_id-start_id) is even multiples of x, (data_id - y)//x//2 * x + y --> [off_h, off_z, mGrid] 
        # when (data_id-start_id) is odd multiples of x, [off_h, off_z, mGrid] --> [off_h+1, off_z, mGrid_len-1-mGrid]
        remap_t = (data_id - start_id) // x # should be a multiple
        data_id_d2 = remap_t // 2 * x + start_id
        #mGrid = mGrid_len - 1 - (data_id_d2 % mGrid_len) # start with max mGrid
        off_zh = data_id_d2 // mGrid_len
        off_z = off_zh % Z
        #off_h = off_zh // Z
        mGrid = tl.where(remap_t % 2 == 1, data_id_d2 % mGrid_len, mGrid_len - 1 - (data_id_d2 % mGrid_len))
        off_h = tl.where(remap_t % 2 == 1, off_zh // Z + H // 2, off_zh // Z)
        ...
        if IS_DELTA_Q:
            ...
            start_m_delta = mGrid * BLOCK_M
            start_m = (start_m_delta + delta_start - seq_start).to(tl.int32)
        else:
            start_m_delta = 0
            start_m = mGrid * BLOCK_M
        # if we fix seq_len to be 2128 and IS_DELTA_Q is fale
        # max value of mGrid is mGrid_len - 1, (mGrid * BLOCK_M) < 2128
        #if start_m >= seq_len:
        #    return
        if HAS_MULTIPLE_TARGETS:
...
class _RaggedAttentionRelativeBiasFunction(torch.autograd.Function):
    ...
    def forward(
        ...
        grid = lambda meta: (  # noqa E731
            #triton.cdiv(N, meta["BLOCK_M"]),
            meta["NUM_LAUNCH"], # assume smaller than number of data grids
            1, #Z * H,
        )
        stride_sz = 0
        stride_sm = 0
