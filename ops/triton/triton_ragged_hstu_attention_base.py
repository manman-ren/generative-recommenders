...
@triton.jit
def _ragged_hstu_attn_fwd_base_one_block(  # noqa: C901
    start_n,
    seq_len,
    offs_m,
    offs_n,
    ...
    if ATTN_BIAS_TYPE == "fused":
        ...
        if USE_TIME_BIAS:
            ...
            ts_w = tl.load(
                TW + ts, #eviction_policy='evict_last',
        ...
        if USE_POS_BIAS:
            ...
            pos_w = tl.load(
                PW + offs_pos_w, #eviction_policy='evict_last',
                #mask=mask_m[:, None] and mask_n[None, :],
            )
...
@triton.jit
def _ragged_hstu_attn_fwd_base(  # noqa C901
    Q,
    K,
    V,
    seq_offsets,
    ...
    for start_n in range(low, high, BLOCK_N):
        acc += _ragged_hstu_attn_fwd_base_one_block(
            start_n=start_n,
            seq_len=seq_len,
            offs_m=offs_m,
            offs_n=offs_n,
...
class _RaggedAttentionBaseFunction(torch.autograd.Function):
    @staticmethod
    # pyre-ignore[14]
    def forward(
        ctx,
        ...
        _ragged_hstu_attn_fwd_base[grid](
            Q=q,
            K=k,
            V=v,
...
class _RaggedAttentionBaseRelativeBiasFunction(torch.autograd.Function):
    @staticmethod
    # pyre-ignore[14]
    def forward(
        ctx,
